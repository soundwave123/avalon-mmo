...
func test_gateway_config_constants() -> void:
	...
	assert_eq(cfg.LISTEN_PORT_WS, 9001, "default WS listen port must be 9001")
	...
	assert_eq(cfg.DEFAULT_WORLD_ANNOUNCE_PORT, 9200, "default announced world port must be 9200")
...
func test_port_env_parsing_rules() -> void:
	var cfg: GDScript = load(CONFIG_PATH)
	assert_eq(cfg.parse_port("9010", 9001), 9010, "valid override wins")
	assert_eq(cfg.parse_port(" 9010 ", 9001), 9010, "whitespace tolerated")
	assert_eq(cfg.parse_port("", 9001), 9001, "unset keeps default")
	assert_eq(cfg.parse_port("banana", 9001), 9001, "garbage keeps default")
	assert_eq(cfg.parse_port("0", 9001), 9001, "out of range keeps default")
	assert_eq(cfg.parse_port("70000", 9001), 9001, "out of range keeps default")
