...
func test_one_time_password_validates_but_is_flagged_for_change() -> void:
	...
	assert_true(_b(check["ok"]), "the one-time password is a real credential")
	assert_true(_b(check["password_is_otp"]), "and the gateway must be told it is one-time")
...
func test_normal_account_is_never_asked_to_change_its_password() -> void:
	...
	assert_true(_b(check["ok"]))
	assert_false(_b(check["password_is_otp"]), "accounts that predate T-740 default to false")
	assert_eq(
		str(Auth.set_password("tester_settled", "their-own-password", "something-else")["reason"]),
		"not_required",
...
func test_set_password_swaps_the_hash_and_clears_the_flag() -> void:
	...
	assert_true(_b(Auth.set_password("tester_new", "Qk7mRt2vXb9d", "my own password")["ok"]))
	var row := Auth.test_account_row("tester_new")
	assert_false(_b(row["password_is_otp"]), "the flag is cleared in the same write")
	assert_ne(str(row["password_hash"]), before, "a fresh salt+digest replaced the provisioned one")
	assert_true(Auth.validate_login("tester_new", "my own password"), "the chosen password works")
	assert_false(_b(Auth.check_login("tester_new", "my own password")["password_is_otp"]))


func test_the_one_time_password_is_dead_after_it_is_spent() -> void:
	_seed_otp_account("tester_spent", "Qk7mRt2vXb9d")
	assert_true(_b(Auth.set_password("tester_spent", "Qk7mRt2vXb9d", "my own password")["ok"]))
	assert_false(Auth.validate_login("tester_spent", "Qk7mRt2vXb9d"), "OTP no longer logs in")
	assert_eq(
		str(Auth.set_password("tester_spent", "Qk7mRt2vXb9d", "a third password")["reason"]),
		"invalid_credentials",
...
func test_set_password_refuses_a_wrong_one_time_password() -> void:
	...
	assert_eq(
		str(Auth.set_password("tester_wrong", "not-the-otp", "my own password")["reason"]),
		"invalid_credentials",
	)
	assert_true(
		_b(Auth.test_account_row("tester_wrong")["password_is_otp"]),
...
func test_short_or_recycled_passwords_are_rejected_server_side() -> void:
	...
	assert_eq(
		str(Auth.set_password("tester_short", "Qk7mRt2vXb9d", "1234567")["reason"]),
		"password_too_short",
		"7 characters is one short of the minimum",
	)
	assert_eq(
		str(Auth.set_password("tester_short", "Qk7mRt2vXb9d", "Qk7mRt2vXb9d")["reason"]),
		"password_unchanged",
		"keeping the one-time password as your own defeats the whole ticket",
	)
	assert_true(_b(Auth.test_account_row("tester_short")["password_is_otp"]), "nothing was written")
	assert_true(
		_b(Auth.set_password("tester_short", "Qk7mRt2vXb9d", "12345678")["ok"]), "8 is enough"
	)
...
func test_set_password_cannot_touch_an_unknown_or_disabled_account() -> void:
	Auth.set_test_auth_mode("accounts")
	assert_eq(
		str(Auth.set_password("no_such_user", "Qk7mRt2vXb9d", "my own password")["reason"]),
		"invalid_credentials",
	)
	...
	assert_eq(
		str(Auth.set_password("tester_off", "Qk7mRt2vXb9d", "my own password")["reason"]),
		"invalid_credentials",
